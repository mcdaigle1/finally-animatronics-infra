...
resource "aws_security_group" "finally-animatronics_rds" {
  name        = "rds-aurora-sg"
...
resource "aws_secretsmanager_secret" "db_master_credentials" {
  name = "finally-animatronics/dev/master-db-credentials"
...
resource "aws_rds_cluster" "finally-animatronics_aurora" {
  cluster_identifier      = "finally-animatronics-aurora-cluster"
  engine                  = "aurora-mysql"
  engine_version          = "5.7.mysql_aurora.2.11.2"
  master_username         = "admin"
  master_password         = random_password.db_master_password.result
  database_name           = "finally-animatronics"
  db_subnet_group_name    = aws_db_subnet_group.aurora.name
  vpc_security_group_ids  = [aws_security_group.finally-animatronics_rds.id]
...
resource "aws_rds_cluster_instance" "aurora_instances" {
  ...
  cluster_identifier = aws_rds_cluster.finally-animatronics_aurora.id
  instance_class     = "db.r6g.large"
  engine             = aws_rds_cluster.finally-animatronics_aurora.engine
  engine_version     = aws_rds_cluster.finally-animatronics_aurora.engine_version
